...
class Translate : Plugin() {
    ...
    private fun DraweeSpanStringBuilder.setTranslated(translateData: TranslateSuccessData, context: Context) {
        ...
        // Message.content is raw Discord markup, while this builder contains the
        // rendered text. Mentions, links and custom emoji can therefore have a
        // different visible length. Use the raw range only when it is present;
        // otherwise replace the rendered message body rather than using a wrong
        // index based on raw content.
        val sourceStart = this.toString().indexOf(translateData.sourceText, contentStartIndex)
        val replaceStart = if (sourceStart >= 0) sourceStart else contentStartIndex
        val replaceEnd = if (sourceStart >= 0) {
            sourceStart + translateData.sourceText.length
        } else {
            this.length
        }
        this.replace(replaceStart, replaceEnd, translateData.translatedText)
    ...
    private fun translateMessage(text: String, from: String? = null, to: String? = null): TranslateData {
        ...
        val translatedText = protectedText.restore(buildString {
